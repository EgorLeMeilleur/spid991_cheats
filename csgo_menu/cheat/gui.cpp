	...
	case WM_MOUSEMOVE: {
		if (wideParameter == MK_LBUTTON)
		{
			...
			if (gui::position.x >= 0 && gui::position.x <= gui::WIDTH &&
				gui::position.y >= 0 && gui::position.y <= 19) //gui::HEIGHT
				SetWindowPos(
					gui::window,
					HWND_TOPMOST,
					rect.left,
...
bool gui::CreateDevice() noexcept
{
	...
	if (d3d->CreateDevice(
		...
		&presentParameters, &device) < 0) return false;
...
void gui::EndRender() noexcept
{
	...
	device->Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_RGBA(0, 0, 0, 255), 1.0f, 0); //background color
...
void gui::Render() noexcept
{
	...
	ImGui::ColorEdit4("glow team color", globals::glowColorTeam);

	ImGui::Checkbox("glow enemy", &globals::glowEnemy);
	ImGui::ColorEdit4("glow enemy color", globals::glowColorEnemy);

	/*ImGui::Checkbox("chams enemy", &globals::chamsEnemy);
	ImGui::ColorEdit3("chams enemy color", globals::chamsColorEnemy);
